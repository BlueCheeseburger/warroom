...
- **`utils/docComboLayout.ts`** persists `{ paneWidths, outlineOpen, sidebarExpanded }` in `localStorage['warroom-doc-combo-layouts']`, keyed by `comboKeyFor()` — the exact, ordered doc paths of every currently-open pane joined on a unit-separator char. Fewer than 2 resolved paths (or an OC case in pane 0, which has no plain path) returns `null` and nothing persists. The wrapper re-hydrates `paneOutlineOpen`/`paneWeights` from the saved combo whenever `comboKey` changes (`lastComboRef` guard).
- **Divider drag**: a 5px handle between adjacent panes (`onDividerMouseDown`) adjusts `paneWeights[i]`/`paneWeights[i+1]` from mouse delta (`deltaPx / rowWidthPx * totalWeight`), clamped to a 12%-of-total floor each. On mouseup it writes the final weights into the current combo via `saveComboLayout`. Disabled (`dividersEnabled = false`) whenever the "space" outline-layout method has any pane's outline open — that split isn't meant to be user-adjustable.
- **Sidebar override**: `Sidebar.tsx`'s `useCollapsed()` hook now also reads `extraDocPanes`/`view` from the store. Its `toggle()` (fired only by the user's own collapse/expand click — the deterministic force-collapse path calls `setC` directly, bypassing `toggle()`) writes `saveComboLayout(comboKey, { sidebarExpanded: !next })` whenever `extraDocPanes` shows multi-pane is active. The wrapper's sidebar effect applies that override once per combo (`appliedOverrideForRef`), right after its default "just became multi → collapse" dispatch, so an existing override always wins.
- **Toolbar overflow menu**: the wrapper passes `toolbarCompact={openPaneCount > 1}` to every `DocPaneViewer`. When set, the reading-time and send-to-flow toggles drop out of the tool cluster and the two `ToolbarPill`s are replaced by a single `ToolbarOverflowMenu` (⋯, opens on hover with a ~160ms close delay so the pointer can cross the gap). Menu rows keep full text labels; rows flagged `ai: true` (Credibility, Cross-Ex) keep the `.ai-glow-ring`, while the ⋯ trigger itself does not — its contents are a mix of AI and non-AI tools, and ringing it would promise an API call just to open a menu.
- **Saved compare views**: `docComboLayout` also records `paths` + `savedAt` per combo (`rememberComboView`, called from the wrapper's combo-change effect) and exposes `listComboViews()` / `deleteComboView()` / `pruneComboViews()` plus a `COMBO_LAYOUTS_CHANGED` window event that `writeAll` announces. `CompareViewsGroup` in `Sidebar.tsx` renders them under Cases, resolving display names from the live `buildCaseItems` list (so renames propagate) and restoring all panes on click via `setView` + two `setExtraDocPane` calls. Deletion goes through `pushUndoToast(message, onUndo)`. `pruneComboViews` is guarded on a non-empty name map — it deletes, so a partial recents read must not be able to wipe the list (same class of hazard as the `case_folders` incident).
...
**Add flow.** The existing selection word-count bubble (`selBubble`) now also clones the live `Range` (`r.cloneRange()` — the native Range mutates/collapses as the selection changes, so the composer needs a snapshot). When a team is signed in and comments are visible, a small comment-bubble button renders beside the word-count pill; clicking it opens a floating composer (quote preview, textarea, a **Team** / **Only me** segmented toggle defaulting **Team**, Cancel/Post — `⌘Enter` posts, `Esc` cancels) anchored near the selection.

**Visibility is one combined toggle**, not "open the panel" vs "show highlights" as two separate states — `commentsVisible` (`localStorage['warroom-doc-comments-visible']`, default on) gates both the highlight registry and whether the panel/composer can be open at all; turning it off force-closes the panel and cancels any open composer. The `doc-toggle-comments` shortcut (`⌘⇧C`, `shortcutPrefs.ts`) and a dedicated eye-icon toolbar button both flip this same boolean — gated on `focused` like `find-page`, so only the active pane in a multi-pane compare view reacts. A separate **Comments** toolbar pill (mutually exclusive with nothing — it can be open alongside Credibility/Cross-Ex) opens the thread list panel; clicking it while comments are hidden un-hides them first rather than opening a panel the user can't see any highlights for. In `toolbarCompact` (2+ panes open), Comments folds into the same ⋯ overflow menu as Credibility/Cross-Ex, but is **not** flagged `ai: true` there since it doesn't call a model.
