...
- **Retries on failure.** If the summarize call still throws after `fetchWithRetry`'s own transient-HTTP retries, `callAIWithDelayedRetry` (`electron/main.ts`, next to the handler) retries the whole call — prompt, parsing, everything — once after a 30s wait, then once more after 60s, before finally giving up (the renderer falls back to plain tag+cite, per its existing best-effort try/catch). This covers the AI content this handler produces: the per-card summaries (which replace the tagline in the cell) and the tab's AI summary folded from them.
