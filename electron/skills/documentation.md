...
**Tab content tooltip.** Hovering a sheet tab shows a wide, ≤4-line tooltip (the `wide` variant of `FlowTooltip`) summarizing the tab — the top few argument tags (first line of each non-empty cell), computed on hover by `sheetSummary(idx)`. Deliberately a cheap local read of the cells (reading `cellsRef` for the active tab, `sheet.cells` for the rest), **not** an AI call, since it fires on every hover. An empty tab shows "Empty tab · double-click to rename".
