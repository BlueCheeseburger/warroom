...
- **Credibility/Cross-Ex compact mode**: `ToolbarPill` takes a `compact` prop (icon-only, `w-7` square, no label) — the wrapper passes `toolbarCompact={openPaneCount > 1}` down to every `DocPaneViewer`.
