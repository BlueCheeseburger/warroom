...
**Classification — `ai:autoFlowClassify`.** IPC handler in `electron/main.ts` (placed directly after `speechdoc:extractBlocks`), prompt template `electron/prompts/auto_flow_classify.txt`. Takes `{ docs, existingSheetNames, existingColumns, event, variant, clarifications }` and returns `{ ok: true, placements }` or `{ ok: true, question }`. Uses the `'balanced'` model tier. For every card, the prompt is told to decide five things: (1) **column** — match the card's `pocket` case-insensitively against `existingColumns` (e.g. `2NC` and `1NR` both route to a column literally named `2NC/1NR` if that's what's present); (2) **sheet** — match `hat`, then `block`, then `pocket` topically against `existingSheetNames`, proposing a new sheet name (`isNewSheet: true`) when nothing plausibly fits rather than forcing a bad match, and when naming a new sheet **prefer the doc's own recurring shorthand** for a position (a doc that says "Federalism DA" once but "Fism DA" in every tag after gets a "Fism DA" tab); (3) **cite** — a **shortened** cite (`Author 'YY`, or an abbreviated source name if there's no individual author) rather than the verbatim credentials paragraph `ExtractedFlowCard.cite` carries — the handler passes this shortened `cite` straight through to the write step, so no client-side truncation is needed; (4) **respondsTo** — the exact tag text of the card this one directly answers (a perm, a no-link, impact defense), or `null`; (5) **isPlan** — `true` for the single policy plan-text card. The handler coerces `respondsTo` to a trimmed string-or-null and `isPlan` to a boolean; both are best-effort hints that degrade to normal placement if the model omits them.
