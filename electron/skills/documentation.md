...
**Choosing a destination.** The user picks a **new flow** or an **existing flow** (from `flowsIndex`). For a new flow, `inferEventFromPockets()` (`src/lib/autoFlowPlacement.ts`) guesses policy vs. PF from the uploaded cards' pocket labels — policy-style codes (`1AC`, `2NC`, …) vs. PF-style phrases (`Pro Case`, `Con Rebuttal`, `Pro FF`, …) — falling back to the app's current `event` when the docs give no usable signal; the initial `StoredFlowData` is built with `makeDefaultData()` from `FlowView.tsx`. For an existing flow, its **actual current** columns (`customColumns` if set, else the default array for its event/pfOrder) and sheet names are read fresh from `flow_data_<id>` before classification, since the user may have renamed either from the defaults.

**Classification — `ai:autoFlowClassify`.** IPC handler in `electron/main.ts` (placed directly after `speechdoc:extractBlocks`), prompt template `electron/prompts/auto_flow_classify.txt`. Takes `{ docs, existingSheetNames, existingColumns, event, variant, clarifications }` and returns `{ ok: true, placements }` or `{ ok: true, question }`. Uses the `'balanced'` model tier. For every card, the prompt is told to: (1) match the card's `pocket` case-insensitively against `existingColumns` to pick a column (e.g. `2NC` and `1NR` both route to a column literally named `2NC/1NR` if that's what's present); (2) match `hat`, then `block`, then `pocket` topically against `existingSheetNames` to pick a sheet, proposing a new sheet name (`isNewSheet: true`) when nothing plausibly fits rather than forcing a bad match.
...
**Write step.** Runs entirely client-side in `AutoFlow.tsx`'s `commitWrite()`, writing straight to `storage.write('flow_data_<id>', …)` — **not** through the live Yjs sync path (an acknowledged v1 limitation; live-collaborating peers won't see an Auto Flow write until they reload the flow). For each accepted placement: the target sheet is found (or created, for `isNewSheet`/unmatched names) by case-insensitive exact name match; `findColumnIndex()` (`src/lib/autoFlowPlacement.ts`) finds the column by case-insensitive exact match against the flow's real columns (no fuzzy matching on this side — that's entirely the AI's job upstream, since it was told to echo a column back verbatim); `firstEmptyRow()` scans `cells['<row>-<col>']` for the first unoccupied row 0..`NUM_ROWS`-1. If a column has no room left, the placement is skipped and surfaced in a "N cards didn't fit" summary rather than dropped silently. Both helpers have a headless test, `scripts/test-auto-flow-placement.ts` (included in `npm run test:flows`).
