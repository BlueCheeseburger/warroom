...
**Disabling individual shortcuts**: standalone command-style shortcuts (not core typing/navigation like Enter/Tab/arrows) carry a stable `id` in `GROUPS` and can be toggled off per-user via `src/lib/shortcutPrefs.ts` (`isShortcutDisabled(id)` / `toggleShortcutDisabled(id)`, backed by a `Set<string>` in `localStorage` under `warroom-disabled-shortcuts`). The UI is deliberately understated — no checkbox, no visible toggle affordance — clicking a disableable entry's `<Kbd>` badge in the overlay toggles it (dimmed + `line-through` when off, cursor/title hint on hover only). Every consuming keydown handler calls `isShortcutDisabled(id)` before acting: `global-search`/`shortcuts-overlay` in `App.tsx`; `find-page` in `useInPageFind.tsx`, `SpeechDocViewer.tsx`, and `FlowView.tsx` (one id, three call sites — ⌘F is one conceptual feature); `flow-bold`/`flow-italic`/`flow-underline`/`flow-strike`/`flow-highlight`/`flow-undo`/`flow-redo`/`flow-link`/`flow-move-row`/`flow-sheet-switch`/`flow-sheet-new` in `FlowView.tsx`. `⌘/` (`shortcuts-overlay`) is itself disableable, but Settings → Keyboard Shortcuts calls `setShortcutsOpen(true)` directly, bypassing the check — so disabling the keybinding never locks a user out of the overlay that would let them re-enable it.
