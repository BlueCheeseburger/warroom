...
The UI (`ShortcutsOverlay.tsx`) has two separate controls per shortcut with an `id` — no checkboxes: a `DisableToggle` power icon to the right of the `<Kbd>` badge toggles disabled state (dimmed + `line-through` on the key, icon colored `var(--neg)` while disabled), always visible at low opacity rather than hover-only, since this one's meant to read as a real button, not a hidden affordance. Double-clicking the key badge itself starts a rebind — `onDoubleClick` sets `recordingId`, which mounts a capture-phase (`addEventListener('keydown', onKey, true)`) listener that treats the next non-modifier-only keydown as the candidate binding. `isBindingValid` rejects anything without `mod` or `alt` set — Shift alone isn't accepted, since Shift+letter is just typing a capital letter, and this listener is live while a flow cell could be focused. `findConflict` rejects a combo already claimed by another *enabled* shortcut and names it in the error shown in place of the key badges. Esc while recording cancels the recording (not the overlay); a second Esc then closes it. A customized id (`hasCustomBinding`) shows a low-opacity "reset" text link (hover-revealed) that calls `resetBinding(id)`. `rebindable` (an id present in `DEFAULT_BINDINGS`) gates the double-click handler and the "Double-click to change" tooltip; `s.id` alone (rebindable or not) gates the `DisableToggle`, so `flow-move-row`/`flow-sheet-switch` get a disable button but no double-click.
