...
**Optional AI card summaries — `ai:autoFlowSummarize`.** A checkbox on the target step (**off by default**, `summarize` state) replaces each card's `tag + cite` cell content with a short AI summary of the card built from its tagline **and its evidence body**, capped to strictly fewer words than the tagline itself.
...
- **Pass 1 (originals):** every card with `respondsTo == null` is placed first, at the `firstEmptyRow()` of its column, so its cell address is known before answers are placed. A `placedByTag` map records where each card landed, keyed by its tag.
- **Pass 2 (answers):** each `respondsTo` card looks up the card it answers in `placedByTag`. If that card is on the **same sheet**, a **different column**, and its row is free in this card's column, the answer is written on that **same row** (adjacency is the visual link — no arrow). Otherwise it drops to `firstEmptyRow()` and a `FlowArrow` (`{ id, from: "ri-ci", to: "ri-ci" }`, pushed onto the sheet's `arrows`) is drawn from the answered card's cell to it — so a second card answering the same one still shows its connection. Cross-sheet answers (arrow endpoints are per-sheet cell keys) and answers whose target wasn't placed just get normal placement, no arrow.
