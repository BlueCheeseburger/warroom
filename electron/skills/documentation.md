...
| Always open the outline | Off by default — the outline auto-opens only for the first doc opened each session (`outlineAutoShownThisSession` module flag), then remembers whatever state the user leaves it in. On forces it open for every doc. `localStorage['warroom-doc-auto-outline']`, checked live in the outline-auto-show branch — no event needed since it only applies as a doc loads. |
