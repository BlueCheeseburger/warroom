...
- **Summary cells are marked.** `SheetData.aiCells?: string[]` holds the cell keys written from a summary; `FlowView` renders those cells with the `.ai-glow-ring` class (the shared blue→pink AI ring). `clearAiCell()` drops a key on the first user edit of that cell (the content is theirs now), `moveCell` swaps ring membership with the content, and `insertRowBetween` bumps the keys along with the cells/arrows. Known limitation: `aiCells` isn't carried in the Yjs `FlowDocData` shape, so the ring doesn't survive a flow being promoted to live collaboration.
