...
**Tab content tooltip + truncation.** Long tab names truncate with an ellipsis (`SheetTab`'s name button is `w-full truncate` inside a `flex-1 min-w-0` `FlowTooltip` wrapper — the `min-w-0` is what lets a flex child actually shrink below its content instead of overflowing). Hovering the tab shows a wide, ≤4-line tooltip (`FlowTooltip`'s `wide` variant): the tab's **full name** on the first line, then a content preview. If `sheet.aiSummary` is set (Auto Flow's opt-in AI card-summary feature folds its per-card summaries into this field at write time — see the Auto Flow section), that real AI-written blurb leads the preview, prefixed `✨`. Otherwise it falls back to the top ~3 argument tags (first line of each non-empty cell), computed on hover by `sheetSummary(idx)` — a cheap local read of the cells (reading `cellsRef` for the active tab, `sheet.cells` for the rest), **not** an AI call, since it fires on every hover. An empty tab shows just its name plus "Empty".
