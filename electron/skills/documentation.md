...
**Go Live lives inside the Share panel**, not as its own toolbar button — there used to be a separate "Collaborate" icon next to Share, but both led to the same panel (Collaborate auto-started a session then opened Share so inviting someone was one motion), so they were combined into a single **Share** button. Opening it shows a "Go live" action (when signed into a team and not already live) that starts a shared realtime session — two or more teammates type into the same flow at once and see each other's edits appear **character-by-character**, like Google Docs. Once live, that same panel shows a live-session banner instead of the "Go live" prompt, so inviting someone is still one continuous flow. A green "Live" pill in the toolbar (not a button — just status) shows who else is present and lets you leave the session; each teammate's active cell is ringed and tagged in their color.

**Sharing a flow always goes live first.** `getData` in the `SharePanel` call site (`FlowView.tsx`) calls `startLiveCollab()` before building the share payload whenever the flow isn't already live and a team is signed in — there's no more "send a frozen, independent copy" path for a flow. This closes a real gap: without it, a plain share handed the recipient a brand-new `flowId` (a genuinely separate copy, per the chat-import branch in `ChatMessage.tsx`/`Chat.tsx` that only reuses the sender's `flowId` when `attachment.data.live` is true) — so if both people happened to have their own copy open at once, there was no shared document to reconcile, just two flows silently diverging. Always sharing live means every recipient joins the exact same doc from the start, so that divergence can't happen. If `startLiveCollab()` fails (offline, promote error), `getData` falls back to a plain snapshot rather than pointing the recipient at a doc that was never actually promoted — check its own return value, not the `live` closure variable, since a `setLive(true)` inside that same call won't be visible to the still-executing closure. The "Send to" step shows a one-line note ("Sending starts a live session…") whenever this is about to happen so it isn't a silent surprise.
