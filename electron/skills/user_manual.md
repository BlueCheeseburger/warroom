...
3. **Choose the body (optional).** Click paragraphs to narrow the card down to just those. **Leave everything unselected and Warroom AI just uses the whole article** — you don't have to pick anything to continue. Paragraphs go in whole; irrelevant parts get shrunk to small text rather than dropped. Open the **Pictures** dropdown to add any images (the ones Warroom AI thinks belong to the article are marked "suggested").
...
6. **Review & fix.** Pick a tagline or write your own, and edit the cite/year. The words themselves never change (the body stays verbatim). To adjust the emphasis you have three tools, no fiddly manual formatting:
   - **Highlight density** (Less / Medium / More) — show just the most essential highlights or the full set Warroom AI found. Instant and free; no extra AI call, so flip between them as much as you like.
   - **Color swatches** — recolor the highlighting that's already there.
   - **"Change something?" box** — tell Warroom AI what to fix in plain English ("underline less", "highlight the statistics", "don't shrink the last paragraph") and hit **Refine**. It re-cuts the card with your change and keeps everything you didn't ask it to touch. This one does cost an AI call.
...
**New flows are named after the aff.** Warroom AI reads the 1AC and names the flow after the aff being read — "Single Payer", not the file name — since that's how you'd refer to the round whichever side you were on. If the docs don't give it a usable read on what the aff is (a neg-only upload), the flow is named after today's date instead. It never guesses from the file name. Rename it any time by double-clicking the tile in the Flows grid.

**Nothing is written unless cards actually landed.** If Warroom AI comes back without a usable placement for a single card, no flow is created at all — you get an error saying so and land back on the sort step to try again. Previously this produced a flow containing one blank tab and no explanation.
...
**How the parser picks a tab.** The most specific heading wins. On a **neg** doc that's the position hat — "Cap K", "States CP", "Midterms DA". On an **aff case file** it's the block, because a case file is built the other way round: every card is hatted with the aff ("1AC---Single Payer") and blocked with the advantage ("1AC---Advantage 1---Economy"). So an advantage heading beats the hat — your advantages become the tabs, and the aff becomes the **flow's name**.
...
- **Fewer buttons per pane**: with 2+ panes open, each pane's toolbar folds Reading time, Send to flow, Credibility, and Cross-Ex into a **⋯** menu (hover it to open) so the doc name and the essentials still fit. They keep their full labels inside the menu — the Highlight readability slider (below) moves in there too, so you never see two separate **⋯** buttons in the same pane.
...
- **Comments**: select text and click the comment bubble that appears (or press **⌘⌥M** / **Ctrl+Alt+M** — the same shortcut Google Docs uses) to leave a note on it, Google-Docs style. **Comment on a whole card**: hover over a card's tag line and click the small comment icon that appears in the margin — no need to select any text, the whole card gets highlighted and commented on at once. Comments go to your **team** by default — everyone signed into the same team sees them, live, while you're both viewing the doc — or pick **Only me** to keep one just for yourself. The highlighted text gets a light purple wash, deliberately different from the document's own evidence highlighting, so it never looks like part of the original doc. A single plain **comment-icon** toolbar button (no AI ring — commenting doesn't call a model) opens the Comments panel listing every thread; click one to jump straight to its highlighted text, which flashes briefly so you can find it. Click the button again (or the panel's own ×) to close the panel and hide every highlight together.
  - **Reply**: click **Reply** under any comment to keep the discussion in one thread instead of starting a new comment — replies collapse to "N replies" until you expand them.
  - **Resolve**: hover a comment and click the checkmark to mark it resolved (anyone on the team can do this, not just whoever wrote it) — it dims, drops its highlight, and moves into a collapsed "resolved" section. Click again to reopen it.
  - **@Mention**: type **@** in a comment or reply to bring up the same mention picker chat uses and tag a teammate by name — it renders as a highlighted chip so it's easy to spot who's being looped in.
  - Only you can delete your own comments (deleting a comment deletes its replies too). Requires being signed into a team.
...
- **Highlight readability**: a small **⋯** button in the toolbar (next to Share) opens a one-slider menu for how muted the doc's yellow/cyan/green highlighting looks — drag it toward **Word's colors** for the exact, full-saturation highlighter colors you're used to, or toward **Muted** for a softer, easier-on-the-eyes look (default sits in between). Applies live to any doc already open, in every pane at once. Only affects light-rendered pages (light theme, or dark mode with "Keep speech docs light" on, which is the default) — a genuinely dark page already dims highlights for readability on its own. The same menu is also in the Google Drive doc viewer.
...
- **Saving a skill asks first**: when Warroom AI wants to save something for itself to remember (a `write_skill` call), a card appears above the message box showing the skill's name and the exact text it wants to save, and it waits for you to hit **Save skill** or **Don't save**. Nothing is written until you choose. This is the only AI action that stops to ask, and it does so because a saved skill is loaded back into Warroom AI's memory in later chats — while the AI also reads web pages you may never see. The prompt means a web page can't quietly talk it into leaving itself instructions that outlast the conversation. Declining is final for that request; it won't retry.
...
- **Invite code**: moved out of the header; it's in Room Settings (gear icon) now, alongside **Leave room**. Team owners also get **Reset invite code** there — it issues a fresh code and immediately stops the old one from working, which is what to do if a code leaked or someone left the team. Current members stay in and the whole message history stays readable; only future joins are affected. DM/group DM settings (gear icon while in a DM) has its own **Leave conversation** button.
...
- **Encryption**: every message and shared attachment is encrypted on your device (AES-256-GCM) before it's sent, so the cloud server only ever stores scrambled ciphertext — a leak of just the message data reveals nothing. The key comes from a separate secret held with the team, not from the invite code — that's what lets an owner reset a leaked invite code without making old messages unreadable. Note this is not zero-knowledge encryption: that secret is also stored on the server, so it's strong protection against a data leak, not a guarantee that the service operator can't read messages. Warroom AI never reads your team-chat history.
...
| Adaptive reading pace | On by default. Auto-scroll slows through dense highlighted cards and speeds up through sparse context, instead of scrolling at one constant speed the whole way through — see Reading time & auto-scroll above. |
