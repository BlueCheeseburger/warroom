...
4. **Tell Warroom AI the plan.** Type what you're using the card for, pick a **highlight color (yellow / cyan / neon green)**, and pick a **cut style**: **Flow** (default) scatters highlights across many short whole words/phrases throughout the underline — modeled on real competitive Verbatim cards, skipping only connector/filler words, never truncating a word. **Lay** highlights fewer, whole contiguous phrases instead, readable at a glance for a lay judge. Then click **Cut card**.
