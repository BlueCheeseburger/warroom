...
const ABBREVIATION_STYLES: Record<'lay' | 'flow', string> = {
  flow: `FLOW STYLE (default): highlight ONLY the front 1-3 letters of each substantive word (noun/verb/adjective) — never the whole word. Every fragment must start at the beginning of a word (never a random mid-word slice, e.g. "in" plucked out of "administrative" is wrong). Skip function words entirely (a, the, of, in, on, at, that, which, and, or, to, is, was, be) — don't highlight even their first letter, since they carry no argumentative content.
Worked example on ordinary prose (not just proper nouns) — for the sentence "the rule will result in many eligible patients losing or being denied Medicaid coverage," a good flow-style highlight array is ["resul", "elig", "los", "den", "Medicaid cov"] — NOT ["losing or being denied Medicaid coverage"] (that is a whole phrase, not abbreviated) and NOT ["result", "eligible"] (those are whole words, not clipped). Proper-noun example: for "North Korea" highlight only "No" and "Ko".
Produce MANY short fragments — dozens across a full card body is normal. If any single highlight entry is more than 4-5 words long, you have not abbreviated enough; split it into individual word-fragments instead. Target: the highlighted characters should add up to roughly 15-25% of the underlined text's character count — noticeably less than lay style, not more. The result will not read as a grammatical sentence — that is expected and correct, this is flow shorthand, not prose.`,
