...
/**
 * Retry a failing call after 8s, then 30s, then 60s (4 attempts, ~100s of
 * silence in the worst case).
 *
 * ONLY for AI calls the user has no way to retry themselves. If the failure path
 * puts them back on a screen where the trigger button still exists, do NOT wrap
 * it — they'd sit through a minute and a half of nothing before seeing an error
 * they could have re-fired in one click. Every handler was audited against this;
 * the only three that still qualify are:
 *
 *   - `ai:summarizeFlowSheet`   — fires on tab hover, no button, silently falls
 *                                 back to the plain tag list.
 *   - `ai:crossExTraps`         — auto-runs from a useEffect when the drill opens.
 *   - `chat:generateGeminiTitle`— names a chat session in the background; the
 *                                 user never sees it run.
 *
 * Everything else (Auto Flow, Round Analysis, card cutter, cross-ex questions,
 * scouting, Impact Calc, Outweigh, Impact Library, flow import, …) renders its
 * error next to a live retry control and calls the model directly.
 *
 * This sits ABOVE `fetchWithRetry`, which already absorbs a single transient
 * 429/5xx inside one request. See CLAUDE.md's "AI call retries + error
 * surfacing" rule.
 */
...
ipcMain.handle('ai:truncationChoice', (_e, { id, proceed }: { id: number; proceed: boolean }) => {
  const resolve = pendingTruncationAsks.get(id);
  if (resolve) { pendingTruncationAsks.delete(id); resolve(!!proceed); }
  return true;
});

/**
 * Thrown when the user declines to send a shortened input. This is a CHOICE, not
 * a failure: preload matches this opening phrase and suppresses the AI error
 * toast for it, so the only feedback is the feature's own inline message.
 */
const TRUNCATION_DECLINED =
  'Cancelled — the input was too long to send in full and you chose not to send a shortened version. Nothing was sent to Warroom AI.';

async function capForPrompt(text: string, limit: number, label: string): Promise<string> {
  const s = String(text ?? '');
  if (s.length <= limit) return s;

  // No window to ask (shouldn't happen — these are all renderer-initiated), so
  // don't hang forever waiting on an answer nobody can give.
  if (!mainWin) return s.slice(0, limit);

  const id = nextTruncationAskId++;
  const proceed = await new Promise<boolean>((resolve) => {
    pendingTruncationAsks.set(id, resolve);
    mainWin!.webContents.send('ai:confirmTruncation', { id, label, kept: limit, total: s.length });
    // If the window goes away mid-question, fail closed rather than silently
    // spending the call on partial input.
    mainWin!.once('closed', () => {
      if (pendingTruncationAsks.delete(id)) resolve(false);
    });
  });
...
ipcMain.handle('ai:extractCards', async (_e, filePath: string) => {
  ...
  const prompt = await renderPrompt('card_extraction', { DOCUMENT_TEXT: await capForPrompt(text, 60000, 'this document') });
...
ipcMain.handle('ai:cutterReadSource', async (_e, filePath: string) => {
  ...
  const numbered = await capForPrompt(rawParagraphs.map((p, i) => `[${i}] ${p}`).join('\n'), 90000, 'this article');
...
// fragments, not words, so grammar is expected to break somewhat. Lay style keeps
// whole words/phrases highlighted so a lay judge can read the emphasis by eye.
const ABBREVIATION_STYLES: Record<'lay' | 'flow', string> = {
  flow: `FLOW STYLE (default): abbreviate ONLY using real, commonly recognized shorthand a debater would actually write — e.g. "administrative" → "admin", "government" → "gov't", "information" → "info", "North Korea" → "No Ko". Do NOT invent unnatural letter-truncations of ordinary words nobody abbreviates — "denied" → "den", "required" → "requir", "result" → "resul" are NOT real abbreviations and must never appear. If a word has no natural, recognizable short form, leave it whole (or don't highlight it) rather than chopping it just to save space. It's fine, and expected, for many words in a highlighted span to stay completely unabbreviated.
The result does not need perfect grammar when read back, but it DOES need to remain a somewhat readable sentence structure — someone glancing at just the highlighted portion should be able to tell what claim is being made. Keep short connector/verb words (is, has, must, will, causes, means) whole; abbreviation is for genuinely long, recognizable nouns and technical/institutional terms, not every word in the sentence.
SENTENCE SCOPE: keep highlighting concentrated in the same 1-3 sentences containing the card's core claim — the same sentences a lay-style cut would pick as most important. If a sentence isn't one of those 1-3, leave it completely unhighlighted, even though individual words in it could technically be shortened. Do not scatter highlight fragments across every sentence of the underlined text.
HARD CAP: highlight AT MOST ONE phrase per underlined sentence — pick only the single most essential clause in each key sentence and highlight nothing else in that sentence (an abbreviated word split into its own array entry, like "admin" next to a whole-word entry, still counts as one phrase together, not two). Never add a second or third highlighted phrase within the same sentence just because more words could technically be marked.
LESS TOTAL HIGHLIGHTING THAN LAY, NOT MORE: flow's total highlighted character count must come out LOWER than a lay-style cut of the same passage would, even though flow uses shorter, abbreviated fragments instead of whole words. If you finish and your highlighted text is close to or larger than what lay style would have picked, you have failed this instruction — go back and drop entries or abbreviate them further until the total is clearly smaller.
CRITICAL MECHANICAL RULE — every highlight array entry, including abbreviated ones, MUST be an exact unmodified slice of the body text. You are NEVER allowed to write the abbreviated spelling into a combined string — the source text still says "administrative", not "admin", so a string like "admin process" does not exist anywhere in the body and will silently fail to highlight anything. Instead, when you want an abbreviated word next to whole words, split them into SEPARATE array entries: one entry that is just the word's own prefix (e.g. "admin", which IS a literal substring of "administrative"), and a separate entry for the following whole-word span. Example: for "the administrative process required to demonstrate compliance," correct output is two entries — "admin" and "process required to demonstrate compliance" — NOT one merged entry "admin process required to demonstrate compliance".
Good example: for "the rule will result in many eligible patients losing or being denied Medicaid coverage," a reasonable flow highlight is the single entry "losing or being denied Medicaid cov" (only the trailing "coverage" is clipped to "cov" IN PLACE — the rest of the word is simply omitted from the end of the slice, not respelled — so this remains a valid exact substring; everything before it stays whole because "losing"/"denied"/"eligible" have no standard abbreviation).`,
...
ipcMain.handle('ai:teamSummary', async (_e, {
  ...
  // No withDelayedRetry: on failure OpponentProfile.tsx keeps the scout card's
  // "Regenerate" button on screen next to the error, so the user can retry
  // themselves — see CLAUDE.md's "AI call retries + error surfacing" rule.
  //
...
ipcMain.handle('ai:crossExQuestions', async (_e, {
  ...
  try {
    ...
    const highlighted = await capForPrompt(highlightedText ?? '', 40000, 'the highlighted text');
    const full = await capForPrompt(fullText ?? '', 60000, 'this document');
...
ipcMain.handle('ai:impactLibraryDraft', async (_e, params: { source: string; event?: string }) => {
  try {
    const source = await capForPrompt(params?.source ?? '', 60000, 'the source text');
...
ipcMain.handle('gemini:importFlow', async (
  ...
  try {
    ...
    // Sequential, not .map() — each cap may stop to ask the user first.
    const sheetBlockList: string[] = [];
    for (const s of sheetsIn) {
      ...
      const json = await capForPrompt(JSON.stringify(capped), 12000, `the "${name}" sheet`);
      sheetBlockList.push(`SHEET "${name}":\n${json}`);
    }
    const sheetBlocks = sheetBlockList.join('\n\n');
...
ipcMain.handle('speechdoc:summarizeForAttachment', async (_e, docText: string, fileName: string) => {
  try {
    ...
    const prompt = await renderPrompt('speechdoc_summarize_attachment', {
      ...
      FILE_NAME: fileName,
      DOC_TEXT: await capForPrompt(docText, 400000, 'this document'),
    });
    const raw = await callAI(prompt, 'best', { maxOutputTokens: 32768 });
    return sbOk(raw);
  } catch (e) { return sbErr(e); }
});
