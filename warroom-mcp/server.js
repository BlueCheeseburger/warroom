...
IMPORTANT — don't double-read a skill you already have natively: if the calling app (e.g. Claude Code, Claude Desktop) already has its own installed skill covering the same topic as one of get_skill's names (for example, a "policy-debate" skill covering the same ground as this server's cx_debate skill), prefer reading and using that native skill instead of calling get_skill here for it. Only fall back to this server's get_skill for a topic when no equivalent native skill is available to you. This avoids pulling in two overlapping copies of the same guidance and wasting context on a redundant read.
...
WHAT "CASES" ACTUALLY ARE, AND WHAT THIS SERVER CANNOT SEE: a case in db.json is either built directly in Warroom out of blocks (no file behind it at all) or imported from an OpenCaselist disclosure (cached docx bytes tied to that source URL) — neither is a path into a file already sitting on the user's local disk. The user's own opened/imported speech-doc files (real .docx files, wherever they happen to live on disk) are tracked in the app's renderer-side browser localStorage, not in db.json — this server is a separate Node process with no access to that localStorage, so it cannot see those file paths or their content at all. This server also cannot reach Team Files (files shared with a team over chat): those are stored in Supabase, end-to-end encrypted with a key derived from the team's invite code, which this server never has. So in short: this server never has, and never points you at, a file already on the user's disk or shared with their team — everything it returns is either local app data from db.json/topics.json, or the result of its own live fetch_article request to a URL you give it.
